...
class FloatingSearchAppBarState extends ImplicitlyAnimatedWidgetState<
  ...
  late final TextController inputController = TextController()
    ..addListener(() {
      if (inputController.text != queryNotifer.value) {
        queryNotifer.value = inputController.text;

        _handler.post(
          // Do not add a delay when the query is empty.
          inputController.text.isEmpty ? Duration.zero : widget.debounceDelay,
          () => widget.onQueryChanged?.call(inputController.text),
        );
      }
    });
  ...
  bool get hasFocus => inputController.hasFocus;
  set hasFocus(bool value) => value ? focus() : unfocus();

  String get query => inputController.text;
  set query(String value) => inputController.text = value;
  ...
  void initState() {
    ...
    if (isAlwaysOpened) {
      _isOpen = true;
      postFrame(inputController.requestFocus);
    }
  ...
  void clear() => inputController.clear();
  ...
  Widget _buildBar() {
    ...
    final bar = GestureDetector(
      onTap: () {
        if (isOpen) {
          hasFocus = !hasFocus;
          inputController.moveCursorToEnd();
        } else if (!isAppBar) {
          isOpen = true;
        }
      },
        ...
        child: widget.textFieldBuilder != null
            ...
            : IntrinsicWidth(
                child: TextField(
                  ...
                  focusNode: inputController.node,
                  maxLines: 1,
                  autofocus: false,
